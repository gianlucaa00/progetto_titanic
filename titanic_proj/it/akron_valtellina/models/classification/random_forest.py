...
class RandomForest:
    def __init__(self, X, y, use_pca=False, n_components=None):
        ...
        self._train_columns = None  # Aggiungi questa riga
    ...
    def train(self, n_estimators=100, max_depth=None, min_samples_split=2,
        ...
        **kwargs : parametri aggiuntivi per RandomForestClassifier
        ...
        if self.__use_pca:
            ...
            # Salva le colonne per l'uso in predict
            ...
            # Salva le colonne del training dopo one-hot encoding
            if hasattr(X_train, 'columns'):
                self._train_columns = X_train.columns.tolist()
        ...
        self.__results = {
            ...
            'train_columns': self._train_columns,  # Aggiungi ai risultati
            'params': {
                'n_estimators': n_estimators,
                'max_depth': max_depth,
                'min_samples_split': min_samples_split,
    ...
    def _preprocess_predict_data(self, X_new):
        ...
        if hasattr(X_new, 'columns'):
            ...
            elif hasattr(X_new, 'columns'):
                # Se non abbiamo salvato le colonne, usa tutte quelle disponibili
                self._train_columns = X_processed.columns.tolist()
    ...
    def predict_proba(self, X_new):
        """Predice probabilita su nuovi dati."""
        if self.__model is None:
            raise ValueError("Devi prima addestrare il modello con train()")
