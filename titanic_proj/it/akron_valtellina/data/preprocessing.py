...
class CleanDataset:
    ...
    def imputa_con_knn(self, X_train, X_test, n_neighbors=5, weights='uniform'):
        ...
        for col in categorical_cols:
            if col in encoders:
                ...
                train_rounded = np.clip(np.round(X_train_imputed[col]).astype(int), 0, len(le.classes_) - 1)
                test_rounded = np.clip(np.round(X_test_imputed[col]).astype(int), 0, len(le.classes_) - 1)
                X_train_imputed[col] = le.inverse_transform(train_rounded)
                X_test_imputed[col] = le.inverse_transform(test_rounded)
        ...
        return X_train_imputed, X_test_imputed, stat, imputer, encoders
    ...
    def gestione_na(self, test_size=0.2, random_state=42, soglia_na=50.0,
        ...
        Returns:
        --------
        tuple: (X, y, preprocessing_info, fill_values)
            - X: DataFrame processato
            - y: Series target processata
            - preprocessing_info: dict con info sul preprocessing
            - fill_values: dict con i valori usati per l'imputazione (solo per metodo statistical)
        """
        df = self.replace_strange_missing_values()
        ...
        # 4. Imputazione
        knn_stat = None
        fill_values = {}  # Dizionario per salvare i valori di imputazione
        knn_imputer = None
        knn_le = None

        if imputation_method == 'knn':
            X_train, X_test, knn_stat, knn_imputer, knn_le = self.imputa_con_knn(X_train, X_test, n_neighbors=knn_neighbors)
        else:
            ...
            for col in X_train.columns:
                if X_train[col].dtype == 'object' or X_train[col].dtype.name == 'string' or X_train[
                    ...
                    # Colonne categoriche → moda
                    mode_val = X_train[col].mode()
                    if len(mode_val) > 0:
                        fill_values[col] = mode_val[0]
                    else:
                        fill_values[col] = X_train[col].iloc[0] if len(X_train[col]) > 0 else 0
                elif pd.api.types.is_numeric_dtype(X_train[col]):
                    # Colonne numeriche → media se normale, altrimenti mediana
        ...
        const_eliminate = X_train.columns[X_train.nunique(dropna=False) <= 1]
        ...
        # 7. Prepara info preprocessing
        preprocessing_info = {
            'imputation_method': imputation_method,
            'columns_removed_high_na': cols_eliminate.tolist() if hasattr(cols_eliminate, 'tolist') else list(
                cols_eliminate),
            'n_columns_removed_high_na': len(cols_eliminate),
            'duplicates_removed': int(duplicates_removed),
            'constant_columns_removed': const_eliminate.tolist() if hasattr(const_eliminate, 'tolist') else list(
                const_eliminate),
            'n_constant_columns_removed': len(const_eliminate),
            'final_train_shape': list(X_train.shape),
            'final_test_shape': list(X_test.shape),
            'final_shape': list(X.shape) if X is not None else None
        }

        if knn_stat:
            preprocessing_info['knn_stats'] = knn_stat

        return X, y, preprocessing_info, fill_values, knn_imputer, knn_le
    ...
    def trova_outliers(self, method='iqr', threshold=1.5, imputation_method='statistical', knn_neighbors=5):
        ...
        """
        df, y, _, _, _, _ = self.gestione_na(imputation_method=imputation_method, knn_neighbors=knn_neighbors)
